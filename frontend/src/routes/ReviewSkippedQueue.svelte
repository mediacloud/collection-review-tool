<script>
  ...
  async function decideSelectedItem(decision, removalReason = null, skipNote = null) {
    ...
    try {
      ...
      console.error('Error updating skipped-item decision:', err);
    } finally {
      decidingItem = false;
    }
  }
...
<div class="container">
  ...
  <div class="subheader">Skipped Sources ({total})</div>
  ...
  {#if showEditMetadataError}
    <div class="error-banner">{showEditMetadataError}</div>
  {/if}
  ...
  {:else if items.length === 0}
    <div class="empty-card">
      <div class="empty-title">No skipped sources</div>
      <div class="empty-subtitle">
        There are no sources currently marked as <strong>skip</strong> in this project.
      </div>
    </div>
  {:else}
